...
/*LFU caching is an algorithm used to manage cache in computer systems and software applications.
It operates by tracking the frequency of access for each item in the cache, and removing the least frequently used item(s) when the cache becomes full.
When a cache reaches maximum capacity, the LFU algorithm identifies the item(s) with the lowest usage frequency and removes them to accommodate the new item.
If multiple items have the same usage frequency, the algorithm may opt to remove the item that was added first or choose a random item.
LFU caching is particularly advantageous in systems where some cache items are more frequently accessed than others.
 By eliminating the least frequently used items, the algorithm ensures that the cache is filled with the most frequently accessed items, leading to better system performance and reduced reliance on costly operations such as disk I/O or network requests.*/
